...
function DoTx() {
  ...
  if (ng.systemInfo.platform() == ng.systemInfo.OsFamily.Windows) {
    ...
    if (ng.systemInfo.isWine()) {
      regSection.setRegistryValue(
          ...
          product.languagebarrier_proxy_dll, 'native,builtin');
    }
  }

  var patchContentSection = ng.tx.tx().addSection('Copying patch content');
  patchContentSection.removeDirectory('%GAME_PATH%/NOTES DaSH');
  ...
  ng.tx.tx().addExecuteAfterFinish(ng.fs.global().expandedPath(
    'cmd /c "cd /d "%GAME_PATH%" & echo @echo off > boot.bat & echo. >> boot.bat & echo start LauncherC0.exe >> boot.bat"'));
...
var DirectoryPage = function() {
  ...
  this.view.addLabel(
      ...
      ') is installed, containing files like Game.exe and launcher.exe:');
...
ng.window.playBgm(
    {url: ':/userdata/bgm.mp3', loopStart: 0, loopEnd: 3307555});
